...
import 'package:homestayraya/view/screens/newproductscreen.dart';
import '../../config.dart';
...
class _SellerScreenState extends State<SellerScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
          appBar: AppBar(title: const Text("Homestay"), actions: [
            // IconButton(
...
//check permission,get location,get address return false if any problem.
  ...
  void _loadProducts() {
    ...
    http
        .get(
      Uri.parse(
          "${CONFIG.server}/php/loadsellerproduct.php?userid=${widget.user.id}"),
    )
        .then((response) {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsondata['status'] == 'success') {
          ...
          if (extractdata['products'] != null) {
            ...
            titlecenter =
                "No Homestay Found"; //if no data returned show title center
            productList.clear();
          }
        } else {
          titlecenter = "No Homestay Found";
        }
      } else {
        titlecenter = "No Homestay Found"; //status code other than 200
